...
func init() {
	registerRepository("QCE/TXMR_STARROCKS", NewEmrStarrocksInstanceRepository)
}

type EmrStarrocksInstanceRepository struct {
	credential common.CredentialIface
	client     *sdk.Client
	logger     log.Logger
}

func (repo *EmrStarrocksInstanceRepository) GetInstanceKey() string {
	return "target"
}

func (repo *EmrStarrocksInstanceRepository) Get(id string) (instance TcInstance, err error) {
...
func (repo *EmrStarrocksInstanceRepository) ListByIds(id []string) (instances []TcInstance, err error) {
	return
}

func (repo *EmrStarrocksInstanceRepository) ListByFilters(filters map[string]string) (instances []TcInstance, err error) {
	req := sdk.NewDescribeInstancesRequest()
	var offset uint64 = 0
...
func NewEmrStarrocksInstanceRepository(cred common.CredentialIface, c *config.TencentConfig, logger log.Logger) (repo TcInstanceRepository, err error) {
	cli, err := client.NewEmrClient(cred, c)
	if err != nil {
		return
	}
	repo = &EmrStarrocksInstanceRepository{
		credential: cred,
		client:     cli,
		logger:     logger,
	}
